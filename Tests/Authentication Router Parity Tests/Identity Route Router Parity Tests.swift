    ...
    static func routes() throws -> [(name: String, route: Identity.Route)] {
        [
            ...
            (
                ...
                .authenticate(
                    .api(.credentials(.init(username: Fixed.email, password: Fixed.password)))
                )
            ),
            (
                "api.create.request",
                .create(.api(.request(.init(email: Fixed.email, password: Fixed.password))))
            ),
            ("api.delete.request", .delete(.api(.request(.init(reauthToken: Fixed.reauthToken))))),
            (
                "api.email.change.request",
                .email(.api(.change(.request(.init(newEmail: Fixed.newEmail)))))
            ),
            (
                "api.password.reset.request",
                .password(.api(.reset(.request(.init(email: Fixed.email)))))
            ),
            (
                "api.mfa.verify",
                .mfa(
            ...
            (
                "view.create.verify",
                .create(.view(.verify(.init(token: Fixed.token, email: Fixed.email))))
            ),
            ...
            (
                ...
                .email(.view(.change(.confirm(.init(token: Fixed.token)))))
            ),
            ...
            (
                ...
                .password(
                    .view(.reset(.confirm(.init(token: Fixed.token, newPassword: Fixed.password))))
                )
            ),
            ("view.password.change.request", .password(.view(.change(.request)))),
            (
                "view.mfa.verify",
                .mfa(.view(.verify(.init(sessionToken: Fixed.sessionToken, attemptsRemaining: 3))))
            ),
            ...
            (
                ...
                .mfa(
                    .view(
                        .backupCodes(
                            .verify(.init(sessionToken: Fixed.sessionToken, attemptsRemaining: 3))
                        )
                    )
                )
            ),
