...
struct `Token Signing Tests` {
    ...
    @Test
    func `Rejects a forged token presented to the token client`() async throws {
        ...
        let forged = try Identity.Token.Access(
            identityId: Identity.ID(UUID()),
        ...
        await #expect(throws: (any Swift.Error).self) {
            try await client.verifyAccess(try forged.token)
        }
    }
