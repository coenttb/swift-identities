...
struct AuditTests {
    ...
    func testPasswordChangeAudit() async throws {
        ...
        let passwordChanges = audits.filter { audit in
            audit.operation == "UPDATE" && audit.newData?.contains("passwordHash") == true
        }
    ...
    func testApiKeyDeactivationAudit() async throws {
        ...
        let updateAudits = audits.filter { audit in
            audit.operation == "UPDATE" && audit.newData?.contains("\"isActive\": false") == true
        }
    ...
    func testRecentSecurityChanges() async throws {
        ...
        for change in changes {
            #expect(
                ["identities", "identity_api_keys", "identity_totp"].contains(change.tableName)
            )
        }
    }
