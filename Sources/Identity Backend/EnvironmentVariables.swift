...
extension EnvVars {
    ...
    package var identitiesJWTRefreshExpiry: TimeInterval {
        get {
            self["IDENTITIES_JWT_REFRESH_EXPIRY"].flatMap(Int.init).map(TimeInterval.init)
                ?? 2_592_000
        }
    ...
    package var identitiesJWTReauthorizationExpiry: TimeInterval {
        get {
            self["IDENTITIES_JWT_REAUTHORIZATION_EXPIRY"].flatMap(Int.init).map(TimeInterval.init)
                ?? 300
        }
