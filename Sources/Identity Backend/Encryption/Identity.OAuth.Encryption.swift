  ...
  public struct Encryption {
    ...
    public static func encrypt(token: String) throws -> String {
      ...
      guard !encryptionKey.isEmpty else {
        logger.warning(
          "OAuth tokens stored without encryption - set IDENTITIES_ENCRYPTION_KEY in production",
          metadata: [
    ...
    public static func decrypt(token encrypted: String) throws -> String {
      ...
      if encryptionKey.isEmpty {
        if encrypted.hasPrefix("v1:") {
          logger.error(
            "Found encrypted token but no encryption key set",
            metadata: [
              "component": "Identity.OAuth.Encryption"
            ]
          )
      ...
      if !encrypted.hasPrefix("v1:") {
        logger.debug(
          "Token not in encrypted format, returning as-is",
          metadata: [
            "component": "Identity.OAuth.Encryption"
          ]
        )
