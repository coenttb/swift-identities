...
extension Identity.Deletion.Client {
  package static func live(
    sendDeletionRequestNotification: @escaping @Sendable (_ email: EmailAddress) async throws ->
      Void,
    sendDeletionConfirmationNotification: @escaping @Sendable (_ email: EmailAddress) async throws
      -> Void
  ) -> Self {
    ...
    return .init(
      ...
      cancel: {
        ...
        try await db.write { db in
          ...
          guard
            let deletion = try await Identity.Deletion.Record
              .findByIdentity(identity.id)
              .pending
              .fetchOne(db),
