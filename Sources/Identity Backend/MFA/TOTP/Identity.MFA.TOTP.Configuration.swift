    ...
    public struct Configuration: Sendable {
        ...
        public init(
            ...
            guard timeStep > 0 && timeStep <= 300 else {
                throw ConfigurationError.invalidTimeStep(
                    "Time step must be between 1 and 300 seconds"
                )
            }
            guard verificationWindow >= 0 && verificationWindow <= 10 else {
                throw ConfigurationError.invalidWindow(
                    "Verification window must be between 0 and 10"
                )
            }
...
extension Identity.MFA.TOTP.Configuration {
    ...
    public static func standard(issuer: String) -> Self {
        do throws(ConfigurationError) {
            ...
            preconditionFailure("Invalid standard TOTP configuration: \(error.localizedDescription)")
        }
    }
    ...
    public static func secure(issuer: String) -> Self {
        do throws(ConfigurationError) {
            ...
            preconditionFailure("Invalid secure TOTP configuration: \(error.localizedDescription)")
        }
    }
    ...
    public static var test: Self {
        do throws(ConfigurationError) {
            ...
            preconditionFailure("Invalid test TOTP configuration: \(error.localizedDescription)")
        }
    }
}
