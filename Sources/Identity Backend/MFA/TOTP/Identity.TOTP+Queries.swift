...
extension Identity.MFA.TOTP.Record {
    ...
    package static func encryptSecret(_ secret: String) throws -> String {
        ...
        guard !encryptionKey.isEmpty else {
            logger.warning("TOTP secrets stored without encryption - set IDENTITIES_ENCRYPTION_KEY in production", metadata: [
    ...
    package func decryptedSecret() throws -> String {
        ...
        if encryptionKey.isEmpty {
            if self.secret.hasPrefix("v1:") {
                logger.error("Found encrypted TOTP secret but no encryption key set", metadata: [
                    "component": "Identity.MFA.TOTP"
                ])
        ...
        if self.secret.hasPrefix("v1:") {
            ...
            guard let data = Data(base64Encoded: encryptedData) else {
                logger.error("Failed to decode base64 TOTP secret data", metadata: [
                    "component": "Identity.MFA.TOTP"
                ])
            ...
            do {
                ...
                logger.error("Failed to decrypt TOTP secret", metadata: [
                    "component": "Identity.MFA.TOTP",
                    "error": "\(error)"
                ])
                throw TOTPError.decryptionFailed
        ...
        if let data = Data(base64Encoded: self.secret),
           let decoded = String(data: data, encoding: .utf8) {
            if decoded.hasPrefix("\(encryptionKey):") {
                logger.debug("Migrating legacy encrypted TOTP secret", metadata: [
                    "component": "Identity.MFA.TOTP"
                ])
        ...
        logger.debug("Using unencrypted TOTP secret", metadata: [
            "component": "Identity.MFA.TOTP"
        ])
        return self.secret
    }
