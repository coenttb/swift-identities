...
extension Identity.API {
    public static func response(
        ...
        if case .logout = api {
            ...
            if let request {
                logger.info(
                    "[Standalone.logout] Current request cookies: \(request.cookies.all.map { "\($0.key)=\($0.value.string.prefix(20))..." }.joined(separator: ", "))"
                )
            }
            ...
            if isFormSubmission {
                ...
                logger.info(
                    "[Standalone.logout] Response cookies after expire: \(response.cookies.all.map { "\($0.key)=\($0.value.string.prefix(20))..., expires=\($0.value.expires?.description ?? "nil"), maxAge=\($0.value.maxAge?.description ?? "nil"), path=\($0.value.path ?? "nil")" }.joined(separator: ", "))"
                )
                ...
                response.expire(cookies: .identity)
                logger.info(
                    "[Standalone.logout] Response cookies after expire: \(response.cookies.all.map { "\($0.key)=\($0.value.string.prefix(20))..., expires=\($0.value.expires?.description ?? "nil"), path=\($0.value.path ?? "nil")" }.joined(separator: ", "))"
                )
                return response
            }
        }
        ...
        if case .logout(.all) = api {
            ...
            if let request {
                logger.info(
                    "[Standalone.logout.all] Current request cookies: \(request.cookies.all.map { "\($0.key)=\($0.value.string.prefix(20))..." }.joined(separator: ", "))"
                )
            }
            ...
            logger.info(
                "[Standalone.logout.all] Client logout.all completed - all sessions invalidated"
            )
            ...
            if isFormSubmission {
                ...
                response.expire(cookies: .identity)
                logger.info(
                    "[Standalone.logout.all] Response cookies after expire: \(response.cookies.all.map { "\($0.key)=\($0.value.string.prefix(20))..., expires=\($0.value.expires?.description ?? "nil"), path=\($0.value.path ?? "nil")" }.joined(separator: ", "))"
                )
                return response
            } else {
                // API request - return JSON
        ...
        do {
            ...
            if case .authenticate(.credentials) = api {
                ...
                if let abortError = error as? Abort {
                    ...
                    let retryAfterValue: Int? =
                        abortError.status == .tooManyRequests
    ...
    private static func handleMFAAPI(
        ...
        case .totp(let totp):
            ...
            case .confirmSetup(let confirm):
                ...
                throw Abort(
                    .badRequest,
                    reason: "TOTP setup confirmation requires prior setup initiation"
                )
        ...
        case .verify(let verify):
            ...
            case .totp:
                ...
                if isFormSubmission {
                    ...
                    let response = try await Response(
                        status: .seeOther,
                        headers: [
                            "Location": configuration.redirect.loginSuccess(identityId)
                                .absoluteString
                        ]
                    )
            ...
            case .sms, .email, .webauthn:
                throw Abort(
                    .notImplemented,
                    reason: "\(verify.method.displayName) verification not yet implemented"
                )
            }
        }
    }
