...
extension Identity.View {
    ...
    private static func handleMFAView(
        ...
        case .totp(let totp):
            ...
            case .manage:
                ...
                let backupCodesRemaining: Int? =
                    configuredMethods.configured.backupCodesRemaining > 0
                    ? configuredMethods.configured.backupCodesRemaining : nil
                ...
                return try await Identity.Frontend.htmlDocument(
                    ...
                    Identity.MFA.TOTP.Manage.View(
                        ...
                        disableAction: isEnabled
                            ? router.url(
                                for: .api(.mfa(.totp(.disable(.init(reauthorizationToken: "")))))
                            ) : nil,
                        regenerateBackupCodesAction: isEnabled
                            ? router.url(for: .view(.mfa(.backupCodes(.display)))) : nil,
        ...
        case .verify(let challenge):
            ...
            let verifyURL =
                URL(string: fullVerifyURL.path, relativeTo: fullVerifyURL.baseURL) ?? fullVerifyURL
        ...
        case .backupCodes(let backupCodesView):
            ...
            case .verify(let challenge):
                ...
                // Create a clean URL with just the path (no query parameters)
                let verifyURL =
                    URL(string: fullVerifyURL.path, relativeTo: fullVerifyURL.baseURL)
                    ?? fullVerifyURL
