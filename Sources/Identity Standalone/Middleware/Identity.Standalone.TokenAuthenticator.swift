...
extension Identity.Standalone {
    ...
    public struct CookieAuthenticator: AsyncMiddleware {
        ...
        public func respond(
            ...
            let isLogoutRoute =
                request.url.path == "/api/logout" || request.url.path == "/api/logout/all"
            ...
            if let accessTokenCookie = request.cookies["access_token"]?.string {
                do {
                    ...
                    // This handles the case where the database is wiped between sessions
                        ...
                        if !identityExists {
                            logger.debug(
                                "Identity from token no longer exists in database",
                                metadata: [
                                    "component": "CookieAuth",
                    ...
                    if accessToken.shouldRefresh && !isLogoutRoute {
                        ...
                        if let refreshTokenCookie = request.cookies["refresh_token"]?.string {
                            do {
                                let response = try await identity.authenticate.token.refresh(
                                    refreshTokenCookie
                                )
                                let newAccessToken = try await tokenClient.verifyAccess(
                    ...
                    if isLogoutRoute {
                        logger.debug(
                            "Access token invalid on logout route, continuing without refresh",
                            metadata: [
                                "component": "CookieAuth",
                                "path": "\(request.url.path)",
                            ]
                    ...
                    if let refreshTokenCookie = request.cookies["refresh_token"]?.string {
                        do {
                            ...
                            let accessToken = try await tokenClient.verifyAccess(
                                response.accessToken
                            )
                            request.auth.login(accessToken)
                            logger.debug(
